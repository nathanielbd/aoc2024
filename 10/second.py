...
class Trailhead(Tile):
    ...
    def get_score(self, map: Map):
        ...
        get_peaks_(self, map)
        return len(peaks)
    def get_rating(self, map: Map):
        def get_rating_(tile: Tile, map: Map):
            if tile.h == 9:
                return 1
            else:
                next_steps = tile.get_next_steps(map)
                if len(next_steps) == 0:
                    return 0
                return sum([get_rating_(t, map) for t in tile.get_next_steps(map)])
        return get_rating_(self, map)
map = Map(data)
trailheads = [Trailhead(x, y) for y, row in enumerate(map.data) for x, h in enumerate(row) if h == 0]
print(sum([trailhead.get_rating(map) for trailhead in trailheads]))